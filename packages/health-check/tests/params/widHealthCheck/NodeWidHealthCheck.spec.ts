...
describe('NodeWidHealthCheckParam', () => {
  describe('update', () => {
    /**
     * @target NodeWidHealthCheckParam.update Should update the wid status to exists, using node
     * @dependencies
     ...
     */
    it('Should update the wid status to exists, using node', async () => {
    ...
    /**
     * @target NodeWidHealthCheckParam.update Should update the wid status to not exists, using node
     * @dependencies
     * - ergoExplorerClientFactory
     * @scenario
     * - mock return value of node watcher address assets
     ...
     */
    it('Should update the wid status to not exists, using node', async () => {
      const BoxesApiMock = jest.fn();
      jest.mocked(ergoNodeClientFactory).mockReturnValue({
        blockchain: {
          // mock return value of node watcher address assets
  ...
  /**
   * @target NodeWidHealthCheckParam.update Should update the wid status to exists, using node with multiple responses
   * @dependencies
   * - ergoExplorerClientFactory
   * @scenario
   * - mock return value of node watcher address assets
   ...
   * - The widExists should update to true successfully using node api
   */
  it('Should update the wid status to exists, using node with multiple responses', async () => {
    const BoxesApiMock = jest.fn();
    jest.mocked(ergoNodeClientFactory).mockReturnValue({
      blockchain: {
        // mock return value of node watcher address assets

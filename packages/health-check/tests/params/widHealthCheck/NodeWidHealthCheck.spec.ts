import { TestNodeWidHealthCheck } from './TestNodeWidHealthCheck';
import ergoNodeClientFactory from '@rosen-clients/ergo-node';

jest.mock('@rosen-clients/ergo-node');

describe('NodeWidAssetHealthCheckParam', () => {
  describe('update', () => {
    /**
     * @target NodeWidAssetHealthCheckParam.update Should update the wid status to exists, using node
     * @dependencies
     * - ergoExplorerClientFactory
     * @scenario
     * - mock return value of node watcher address assets
     * - mock return value of node address boxes
     * - create new instance of NodeWidHealthCheckParam
     * - update the parameter
     * @expected
     * - The widExists should update to true successfully using node api
     */
    it('Should update the wid status to exists, using node', async () => {
      let firstOut = true;
      jest.mocked(ergoNodeClientFactory).mockReturnValue({
        blockchain: {
          // mock return value of node watcher address assets
          getAddressBalanceTotal: async () => ({
            confirmed: {
              tokens: [
                {
                  tokenId:
                    '10278c102bf890fdab8ef5111e94053c90b3541bc25b0de2ee8aa6305ccec3de',
                  amount: 1200n,
                },
                {
                  tokenId:
                    'e39047fa7025f5eb94f0b1a9d6d4728b5a4270ea1155e4e5b0e265db46589d5c',
                  amount: 1n,
                },
              ],
            },
          }),
          // mock return value of node address boxes
          getBoxesByAddressUnspent: async () => {
            if (firstOut) {
              firstOut = false;
              return [
                {
                  assets: [
                    {
                      tokenId: 'rwtRepoNft',
                      amount: 1n,
                    },
                  ],
                  additionalRegisters: {
                    R4: '',
                  },
                },
                {
                  assets: [
                    {
                      tokenId:
                        '383d70ab083cc23336a46370fe730b2c51db0e831586b6d545202cbc33938ee1',
                      amount: 12n,
                    },
                  ],
                },
              ];
            } else return [];
          },
        },
      } as any);

      // create new instance of ErgoNodeWidHealthCheck
      const assetHealthCheckParam = new TestNodeWidHealthCheck(
        'rwtRepoAddress',
        'rwtRepoNft',
        'address',
        'node_url'
      );

      // update the parameter
      await assetHealthCheckParam.update();
      expect(assetHealthCheckParam.getWidStatus()).toBe(true);
    });

    /**
     * @target NodeWidAssetHealthCheckParam.update Should update the wid status to not exists, using node
     * @dependencies
     * - ergoExplorerClientFactory
     * @scenario
     * - mock return value of node watcher address assets
     * - mock return value of node address boxes
     * - create new instance of NodeWidHealthCheckParam
     * - update the parameter
     * @expected
     * - The widExists should update to false successfully using node api
     */
    it('Should update the wid status to not exists, using node', async () => {
      let firstOut = true;
      jest.mocked(ergoNodeClientFactory).mockReturnValue({
        blockchain: {
          // mock return value of node watcher address assets
          getAddressBalanceTotal: async () => ({
            confirmed: {
              tokens: [
                {
                  tokenId: 'tokenId',
                  amount: 1200n,
                },
              ],
            },
          }),
          // mock return value of node address boxes
          getBoxesByAddressUnspent: async () => {
            if (firstOut) {
              firstOut = false;
              return [
                {
                  assets: [
                    {
                      tokenId: 'rwtRepoNft',
                      amount: 1n,
                    },
                  ],
                  additionalRegisters: {
                    R4: '',
                  },
                },
                {
                  assets: [
                    {
                      tokenId:
                        '383d70ab083cc23336a46370fe730b2c51db0e831586b6d545202cbc33938ee1',
                      amount: 12n,
                    },
                  ],
                },
              ];
            } else return [];
          },
        },
      } as any);

      // create new instance of ErgoNodeWidHealthCheck
      const assetHealthCheckParam = new TestNodeWidHealthCheck(
        'rwtRepoAddress',
        'rwtRepoNft',
        'address',
        'node_url'
      );

      // update the parameter
      await assetHealthCheckParam.update();
      expect(assetHealthCheckParam.getWidStatus()).toBe(false);
    });
  });
});

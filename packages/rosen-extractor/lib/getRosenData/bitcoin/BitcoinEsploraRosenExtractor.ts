...
export class BitcoinEsploraRosenExtractor extends AbstractRosenDataExtractor<BitcoinEsploraTransaction> {
  ...
  get = (transaction: BitcoinEsploraTransaction): RosenData | undefined => {
    ...
    try {
      ...
      const opReturnData = this.parseRosenData(outputs[0].scriptpubkey);
      if (!opReturnData) {
        this.logger.debug(
          baseError + `: Failed to extract data from OP_RETURN box`
        );
        return undefined;
      }
   ...
   */
  parseRosenData = (scriptPubKeyHex: string): OpReturnData | undefined => {
    // check OP_RETURN opcode
    if (scriptPubKeyHex.slice(0, 2) !== '6a') return undefined;
    // check OP_PUSHDATA1 opcode
    if (scriptPubKeyHex.slice(2, 4) !== '4c') return undefined;
    ...
    if (toChainCode >= SUPPORTED_CHAINS.length) return undefined;
    ...
    if (addressLength > 57) return undefined;
    const addressHex = scriptPubKeyHex.slice(42, 42 + addressLength * 2);

...
describe('selectErgoBoxes', () => {
  ...
  /**
   * @target selectErgoBoxes should interact with AsyncIterator successfully
   * @dependencies
   * @scenario
  ...
  it('should interact with AsyncIterator successfully', async () => {
    // Mock an async generator to return 12 utxos paginated
    const mockedBoxes = boxes.slice(0, 12);
    async function* generator() {
      let offset = 0;
      const limit = 2;
      while (true) {
        const page = mockedBoxes.slice(offset, offset + limit);
